...
const DownloaderCard: React.FC<DownloaderCardProps> = ({ item, onDownload, onRemove, onRename }) => {
  ...
  const getStatusBadge = () => {
    switch (item.status) {
      case DownloadStatus.ANALYZING: return <span className="text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded bg-blue-900/40 text-blue-300 animate-pulse border border-blue-800/50">分析中</span>;
      case DownloadStatus.DOWNLOADING: return <span className="text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded bg-amber-900/40 text-amber-300 border border-amber-800/50">下載中</span>;
      case DownloadStatus.COMPLETED: return <span className="text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded bg-emerald-900/40 text-emerald-300 border border-emerald-800/50">已完成</span>;
      case DownloadStatus.ERROR: return <span className="text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded bg-rose-900/40 text-rose-300 border border-rose-800/50">失敗</span>;
      default: return <span className="text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded bg-slate-800/50 text-slate-400 border border-slate-700/50">待下載</span>;
    }
  };
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mb-4">
            <p className="text-[10px] text-slate-500 truncate font-mono flex-1">{item.originalUrl}</p>
            {item.type === MediaType.VIDEO && (
              <span className="text-[9px] bg-purple-900/30 text-purple-300 px-1.5 py-0.5 rounded border border-purple-800/50 font-bold whitespace-nowrap">QuickTime (.MOV)</span>
            )}
          </div>
          ...
          {!isDownloading && item.status === DownloadStatus.COMPLETED && (
             <div className="mb-4 flex items-center gap-2 text-[10px] text-emerald-400 font-medium bg-emerald-900/10 px-2 py-1 rounded-md border border-emerald-900/20">
                ...
                已保存: <span className="font-bold truncate">{item.filename}</span>
             </div>
          )}
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {item.status === DownloadStatus.COMPLETED ? '再次下載' : '立即下載'}
            </button>
            <button
