...
const App: React.FC = () => {
  ...
  const handleSetFolder = async () => {
    ...
    try {
      ...
      if (err.name === 'SecurityError') {
        setFsError("Browser security blocks folder selection in this preview.");
      }
    }
  };
  ...
  const fetchBlobWithMultiProxy = async (id: string, url: string): Promise<Blob> => {
    ...
    const proxies = [{ name: "Direct", fn: (u: string) => u }, { name: "Proxy A", fn: (u: string) => `https://corsproxy.io/?${encodeURIComponent(u)}` }];

    let lastError = '';
    for (const proxy of proxies) {
      try {
        updateStatus(5, `Via ${proxy.name}...`);
        ...
        while(true) {
          ...
          updateStatus(p, `Fetching...`, `${(received / 1024 / 1024).toFixed(1)} MB`);
        }
  ...
  const downloadSingleFile = async (id: string) => {
    ...
    try {
      const blob = await fetchBlobWithMultiProxy(item.id, item.originalUrl);
      if (dirHandle) {
        ...
        if (hasPermission) {
          ...
          setItems(prev => prev.map(i => i.id === id ? { ...i, status: DownloadStatus.COMPLETED, progress: 100, simulatedSpeed: 'Saved to Local' } : i));
          return;
        }
      }
      ...
      setItems(prev => prev.map(i => i.id === id ? { ...i, status: DownloadStatus.COMPLETED, progress: 100, simulatedSpeed: 'Downloaded' } : i));
    } catch (err: any) {
  ...
  const handleDownloadAsZip = async () => {
    ...
    for (const item of targetItems) {
      try {
        ...
        setItems(prev => prev.map(i => i.id === item.id ? { ...i, status: DownloadStatus.ERROR, error: 'Access Denied' } : i));
      }
    }
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 flex flex-col max-w-6xl mx-auto selection:bg-blue-500/30">
            ...
            <section className="bg-slate-800/20 border border-slate-700/50 p-6 rounded-[2.5rem] shadow-2xl backdrop-blur-xl ring-1 ring-white/5">
              ...
              <div className="mt-6 flex flex-col sm:flex-row gap-4 pt-6 border-t border-slate-800/50">
                <button onClick={handleAddUrls} disabled={!urlInput.trim() || isAnalyzing} className="flex-[2] bg-blue-600 hover:bg-blue-500 disabled:opacity-40 text-white font-bold py-4 rounded-2xl shadow-xl transition-all active:scale-[0.98]">
                  {isAnalyzing ? "Analyzing..." : "Analyze & Queue"}
                </button>
                {items.length > 0 && (
              ...
              <section className="space-y-6">
                ...
                <div className="p-6 bg-slate-800/40 border border-slate-700/30 rounded-[2rem] flex flex-col gap-6">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest">Batch Renaming Tools</h3>
                  </div>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {/* Prefix Tool */}
                    <div className="flex flex-col gap-2">
                      <label className="text-[10px] text-slate-500 font-bold uppercase ml-1">Add Prefix</label>
                      <div className="flex bg-slate-900/80 rounded-xl border border-slate-700/50 overflow-hidden">
                        <input type="text" placeholder="e.g. Travel_" value={prefix} onChange={e => setPrefix(e.target.value)} className="bg-transparent text-xs px-3 py-2 outline-none flex-1 font-medium" />
                        <button onClick={applyBulkPrefix} className="bg-blue-600 hover:bg-blue-500 text-[10px] font-bold px-3 transition-colors">Add</button>
                      </div>
                    </div>

                    {/* Search & Replace Tool */}
                    <div className="flex flex-col gap-2 lg:col-span-2">
                      <label className="text-[10px] text-slate-500 font-bold uppercase ml-1">Search & Replace</label>
                      <div className="flex bg-slate-900/80 rounded-xl border border-slate-700/50 overflow-hidden">
                        <input type="text" placeholder="Search..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="bg-transparent text-xs px-3 py-2 outline-none flex-1 font-medium border-r border-slate-700/50" />
                        <input type="text" placeholder="Replace with..." value={replaceValue} onChange={e => setReplaceValue(e.target.value)} className="bg-transparent text-xs px-3 py-2 outline-none flex-1 font-medium" />
                        <button onClick={applySearchReplace} className="bg-indigo-600 hover:bg-indigo-500 text-[10px] font-bold px-4 transition-colors">Replace</button>
                      </div>
                    </div>

                    {/* Utility Tools */}
                    <div className="flex flex-col gap-2">
                      <label className="text-[10px] text-slate-500 font-bold uppercase ml-1">Actions</label>
                      <div className="flex gap-2">
                        <button onClick={applySequence} className="flex-1 bg-slate-700 hover:bg-slate-600 text-[10px] font-bold py-2 rounded-xl border border-slate-600/50 transition-colors">Add # Sequence</button>
                        <button onClick={resetFilenames} className="px-3 bg-rose-900/20 hover:bg-rose-900/40 text-rose-400 text-[10px] font-bold rounded-xl border border-rose-900/30 transition-colors" title="Reset all to original">Reset</button>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center px-4">
                  <h4 className="text-sm font-bold text-slate-500">Queue ({items.length})</h4>
      ...
      <footer className="py-10 text-center border-t border-slate-800/50 opacity-40">
        <p className="text-[10px] uppercase tracking-widest font-bold">OmniDownloader Pro Engine • Browser Native Processing</p>
      </footer>
    </div>
  );
};
